    ...
    for(i_f=0;i_f<FPW;i_f=i_f+1)begin
        if(d_in_hdr[i_f])begin
            if(d_in_tail[i_f]) begin
                ...
                crc_pkt_length_comb[crc_var_multi_comb] = lng(d_in_flit[i_f]);
...
//====================================================================
//---------------------------------Check the fill grades, reorder !most timing critical
//====================================================================
always @(*)  begin
...
//====================================================================
//Assign the targets. Whenever there's a header assign the target crc using the offset
always @(*)  begin
...
if(!res_n) begin
    ...
    for(i_f=0;i_f<FPW;i_f=i_f+1)begin
        ...
        if(crc_data_pipe_in_tail[CRC_DATA_PIPE_DEPTH-2][i_f])begin  //Shift out the fifos with the desired CRCs
        ...
        if(crc_data_pipe_in_tail[CRC_DATA_PIPE_DEPTH-1][i_f])begin    //Finally add the crc
            data_rdy_flit[i_f][128-1:128-32] <= crc_o_fifo_d_out[crc_data_pipe_out_last_stage_target_crc_per_flit[i_f]];
        end
    end
end
end
